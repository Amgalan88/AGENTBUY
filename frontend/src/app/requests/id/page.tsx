...
const MARKET_APP_LABEL: Record<string, string> = {
  any: "Ямар ч маркет",
  taobao: "Taobao",
  pinduoduo: "Pinduoduo",
  "1688": "1688 / Alibaba",
...
const ORDER_STATUS_FLOW: StatusFlow[] = [
...
const STATUS_BRIDGE: Record<string, string> = {
  new: "PUBLISHED",
...
const Pill = ({ tone = "neutral", children }: PillProps): React.JSX.Element => (
  <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium ${toneMap[tone] || toneMap.neutral}`}>
    {children}
  </span>
);
...
const InfoStat = ({ label, value, helper }: InfoStatProps): React.JSX.Element => (
...
export default function RequestDetailPage(): React.JSX.Element {
  ...
  const [request, setRequest] = useState<Order | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadRequest(): Promise<void> {
      try {
        ...
        if (alive) setRequest(data as Order);
      } catch (err) {
  ...
  const totalMnt = useMemo<number>(() => {
    const price = Number(request?.report?.pricing?.grandTotalCny || 0);
  ...
  const stats = [
    {
      ...
      value: (request as { type?: string })?.type === "batch" ? "Багц" : "Нэг бараа",
      helper: (request as { urgency?: string })?.urgency ? URGENCY_LABEL[(request as { urgency?: string }).urgency || "normal"] || "Дундаж" : undefined,
    },
    {
      label: "Барааны тоо",
    ...
    {
      label: "Карго",
      value: (typeof request?.cargoId === "object" && request?.cargoId?.name) || (typeof request?.cargoId === "string" ? request.cargoId : "Тодорхойгүй"),
    },
  ];
      ...
      <div className={`${widthClass} mx-auto px-4 py-10 space-y-6`}>
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Pill tone="neutral">{(request as { type?: string })?.type === "batch" ? "Багц" : "Нэг бараа"}</Pill>
            {(request as { urgency?: string })?.urgency === "high" && <Pill tone="warning">Яаралтай</Pill>}
          </div>
        </header>
          ...
          <div className={`rounded-3xl border px-5 py-5 ${shellClass}`}>
            ...
            <div className="mt-4 space-y-4">
              {items.map((item, index) => {
                const itemWithName = item as typeof item & { name?: string; mark?: string; note?: string; link?: string };
                return (
                  <article
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">
                          #{index + 1} {itemWithName.name || item.title}
                        </p>
                        {itemWithName.mark && (
                          <p className="text-xs text-slate-500">Тэмдэглэл: {itemWithName.mark}</p>
                        )}
                      </div>
                      <div className="flex flex-wrap gap-2 text-[11px] text-slate-600">
                        ...
                        <Pill tone="primary">
                          {MARKET_APP_LABEL[(item as { app?: string }).app || "any"] || (item as { app?: string }).app}
                        </Pill>
                      </div>
                    </div>
                    {itemWithName.note && (
                      <p className="mt-2 text-sm text-slate-600">Тайлбар: {itemWithName.note}</p>
                    )}
                    {itemWithName.link && (
                      <a
                        href={itemWithName.link}
                        target="_blank"
                        rel="noreferrer"
                ...
                <dl className="mt-4 space-y-3 text-sm text-slate-700">
                  <div className="flex items-center justify-between">
                    ...
                    <dd className="font-semibold text-slate-900">
                      {request.report.pricing?.grandTotalCny || "-"}
                    </dd>
                  </div>
                  {request.report.items?.length > 0 &&
                    request.report.items.map((rItem, idx) => {
                      const imgs =
                        ...
                        ((rItem as { image?: string }).image ? [(rItem as { image?: string }).image as string] : []);
                      return (
                        <div key={idx} className="flex flex-col gap-1 border border-slate-200 rounded-xl p-3">
                          ...
                          {(rItem as { note?: string }).note && (
                            <dd className="text-xs text-slate-600">Тайлбар: {(rItem as { note?: string }).note}</dd>
                          )}
                        </div>
                      );
                    })}
                  {(request.report as { link?: string }).link && (
                    <div className="flex flex-col gap-1">
                      <dt className="text-slate-500">Барааны холбоос</dt>
                      <dd>
                        <a
                          href={(request.report as { link?: string }).link}
                          target="_blank"
                          rel="noreferrer"
                  ...
                  {(request.report as { paymentLink?: string }).paymentLink && (
                    <div className="flex flex-col gap-1">
                      <dt className="text-slate-500">Төлбөрийн холбоос (админ/агент)</dt>
                      <dd>
                        <a
                          href={(request.report as { paymentLink?: string }).paymentLink}
                          target="_blank"
                          rel="noreferrer"
                          className="text-emerald-600 underline"
                        >
                  ...
                  {(request.report as { note?: string }).note && (
                    <div className="flex flex-col gap-1">
                      <dt className="text-slate-500">Нэмэлт тайлбар</dt>
                      <dd>{(request.report as { note?: string }).note}</dd>
                    </div>
                  )}
                </dl>
          ...
          <section className={`rounded-3xl border px-5 py-5 ${shellClass}`}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {[0, 1, 2, 3, 4, 5].map((value) => {
                const requestRating = (request as { rating?: number }).rating;
                const active = requestRating === value || rating === value;
                return (
                  <Button
                    key={value}
