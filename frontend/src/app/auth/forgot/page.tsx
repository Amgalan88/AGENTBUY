...
export default function ForgotPasswordPage(): React.JSX.Element {
  ...
  const handleFetchQuestion = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    setError("");
    setLoading(true);
    try {
      const data = await api<{ question: string }>(`/api/auth/secret-question/${phone}`);
      if (data?.question) {
  ...
  const handleVerifyAnswer = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    setError("");
    setLoading(true);
    try {
      const data = await api<{ resetToken: string }>("/api/auth/verify-secret", {
        method: "POST",
        body: { phone, secretAnswer },
      });
      if (data?.resetToken) {
      ...
      const error = err as Error;
      setError(error.message || "Хариулт буруу байна");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetPassword = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      await api("/api/auth/reset-password", {
        method: "POST",
        body: { phone, newPassword: password, resetToken },
      });
      ...
      const error = err as Error;
      setError(error.message || "Сэргээхэд алдаа гарлаа");
    } finally {
      setLoading(false);
    }
  };
